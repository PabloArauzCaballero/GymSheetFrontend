...
import { defaultPolicy, type NotificationPolicy } from './policy';
...
export type NotifyInput = string | Omit<NotificationRequest, 'severity'>;
...
/**
 * The single notification surface of a client. Feature code depends on this
 * object, never on the toast provider. Deduplication, policy and error mapping
 * all live behind it; the adapter decides how a request is rendered (sonner on
 * web, the native toast host on mobile).
 */
export class NotificationEngine {
  ...
  error(errorOrInput: unknown, options?: Omit<NotificationRequest, 'severity' | 'message'>) {
    ...
    reportError(resolved, options?.correlationId, errorOrInput);
  ...
  async promise<T>(operation: Promise<T>, messages: PromiseMessages<T>): Promise<T> {
    ...
    try {
      ...
      this.adapter.show({
        ...
        id,
        message: text,
        duration: this.durationFor('success'),
      });
      return value;
    } catch (error) {
      if (messages.error) {
        ...
        reportError(resolved, undefined, error);
        this.adapter.show({
          severity: 'error',
          id,
          title: resolved.title,
