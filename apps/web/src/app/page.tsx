...
import { Activity, ArrowRight, ArrowUpRight, BarChart3, Check, Dumbbell, ShieldCheck, Sparkles, TrendingUp, Users2 } from 'lucide-react';
    ...
    <div className="relative mx-auto w-full max-w-xl" aria-label="Vista previa de una sesión en GymSheet">
      ...
      <div className="relative overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-lowest)] shadow-[var(--shadow-lg)]">
        ...
        <div className="grid gap-5 p-6">
          <div className="flex items-start justify-between"><div><p className="text-sm text-[var(--text-muted)]">Empuje · Semana 6</p><p className="mt-1 text-2xl font-semibold">Press de banca</p></div><span className="grid size-11 place-items-center rounded-[var(--radius-lg)] bg-[var(--volt)] text-[var(--accent-contrast)]"><Dumbbell className="size-5" /></span></div>
          <div className="grid grid-cols-3 gap-2">{[['Peso','80 kg'],['Reps','8'],['RIR','2']].map(([label,value]) => <div className="rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-low)] p-3" key={label}><p className="data-label text-[var(--text-disabled)]">{label}</p><p className="mt-2 text-lg font-semibold tabular-nums">{value}</p></div>)}</div>
    ...
    <div className="relative isolate flex min-h-dvh flex-col bg-[var(--background)]">
      ...
      <main className="flex-1">
          ...
          <section className="mx-auto grid max-w-6xl gap-5 px-5 py-20 sm:grid-cols-3 sm:px-8" id="producto">
            {FEATURES.map((feature, index) => (
              ...
              >
                ...
                <h2 className="text-lg font-semibold tracking-[-0.02em]">{feature.title}</h2>
          ...
          <section className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
            <div className="grid overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-lowest)] lg:grid-cols-[1.15fr_0.85fr]">
              <div className="p-8 sm:p-12">
                ...
                <h2 className="mt-5 text-3xl font-semibold tracking-[-0.03em] sm:text-5xl">Menos administración. Más intención.</h2>
              ...
              <div className="grid gap-px bg-[var(--border-subtle)] sm:grid-cols-2 lg:grid-cols-1">
                {[{ icon: BarChart3, title: 'Progreso legible', text: 'Métricas que explican sin abrumar.' }, { icon: ShieldCheck, title: 'Privacidad por diseño', text: 'Permisos validados y control sobre lo que compartes.' }].map(({ icon: Icon, title, text }) => <div className="bg-[var(--surface-low)] p-7" key={title}><Icon className="size-5 text-[var(--accent-ink)]" /><h3 className="mt-5 font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{text}</p></div>)}
              </div>
            </div>
          </section>
        </Reveal>
        ...
        <section className="mx-auto max-w-6xl px-5 pb-24 sm:px-8" id="planes">
          <Reveal><div className="text-center"><p className="data-label text-[var(--accent-ink)]">Planes simples</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] sm:text-5xl">Empieza hoy. Crece sin cambiar de sistema.</h2><p className="mx-auto mt-5 max-w-xl text-[var(--text-muted)]">Opciones claras para personas y una propuesta adaptable para gimnasios.</p></div></Reveal>
          <div className="mt-10 grid gap-5 lg:grid-cols-3">{PLANS.map((plan) => <Reveal key={plan.name}><article className={`relative h-full rounded-[var(--radius-xl)] border p-7 ${plan.featured ? 'border-[var(--volt)] bg-[color-mix(in_srgb,var(--volt)_7%,var(--surface-lowest))]' : 'border-[var(--border-subtle)] bg-[var(--surface-lowest)]'}`}>{plan.featured ? <span className="absolute right-5 top-5 rounded-full bg-[var(--volt)] px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--accent-contrast)]">Recomendado</span> : null}<p className="data-label text-[var(--text-muted)]">{plan.name}</p><p className="mt-6 text-3xl font-semibold tracking-[-0.03em]">{plan.price}</p><p className="mt-4 min-h-12 text-sm leading-6 text-[var(--text-muted)]">{plan.copy}</p><ButtonLink className="mt-7 w-full" href={plan.name === 'Gimnasio' ? '/gimnasios' : '/register'} variant={plan.featured ? 'primary' : 'secondary'}>{plan.name === 'Gimnasio' ? 'Conocer opciones' : 'Elegir plan'}</ButtonLink></article></Reveal>)}</div>
        </section>
            ...
            <section className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
              <div className="mb-6 flex items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="text-2xl font-semibold tracking-[-0.02em]">Sedes cerca de ti</h2>
                </div>
                <Link
        ...
        <section className="mx-auto max-w-3xl px-5 pb-24 sm:px-8" id="preguntas">
          <Reveal><div className="text-center"><p className="data-label text-[var(--accent-ink)]">Preguntas frecuentes</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em]">Todo claro antes de empezar.</h2></div></Reveal>
